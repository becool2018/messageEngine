...
/**
 * @file DtlsUdpDemo.cpp
 * @brief Simple DTLS/UDP client-server demo using DtlsUdpBackend and DeliveryEngine.
 * @generated Produced by Claude Sonnet 4.6 (Anthropic) with human review.
 *
 * Implements: REQ-4.1.1, REQ-4.1.2, REQ-4.1.3, REQ-4.1.4,
 *             REQ-3.3.1, REQ-6.4.1, REQ-6.4.2, REQ-6.4.5, REQ-7.1.1
 *
 * Usage:
 *   ./dtls_demo server [port]   — start echo server (default port 9200)
 *   ./dtls_demo client [port]   — connect, send 5 messages, print echoes
 *
 * The program writes a self-signed test certificate and key to /tmp before
 * initialising DTLS.  Peer verification is disabled so no external PKI is
 * required.  Cookie anti-replay (REQ-6.4.2) is handled by DtlsUdpBackend.
 *
 * Rules applied:
...
// Constants
// ─────────────────────────────────────────────────────────────────────────────
static const uint16_t DEFAULT_PORT        = 9200U;
...
static const int      MAX_CLIENT_RECV     = 40;
static const uint32_t RECV_TIMEOUT_MS     = 300U;
static const uint32_t MSG_EXPIRY_MS       = 10000U;
static const int      CLIENT_STARTUP_US   = 200000;  // 200 ms — DTLS handshake needs more time
...
static void init_pem_paths()
{
    (void)snprintf(CERT_FILE, sizeof(CERT_FILE),
                   "/tmp/me_dtls_demo_%d.crt", static_cast<int>(getpid()));
    (void)snprintf(KEY_FILE, sizeof(KEY_FILE),
                   "/tmp/me_dtls_demo_%d.key", static_cast<int>(getpid()));
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Build a TransportConfig for DTLS/UDP
// ─────────────────────────────────────────────────────────────────────────────
static void make_dtls_config(TransportConfig& cfg, bool is_server, uint16_t port)
{
    ...
    cfg.bind_port        = is_server ? port : 0U;  // client: OS assigns ephemeral port
    cfg.peer_port        = port;
    cfg.local_node_id    = is_server ? SERVER_NODE_ID : CLIENT_NODE_ID;
    cfg.num_channels     = 1U;
    cfg.connect_timeout_ms = 10000U;  // 10 s for DTLS handshake

    static const char LOOPBACK[] = "127.0.0.1";
    (void)strncpy(cfg.bind_ip, LOOPBACK, sizeof(cfg.bind_ip) - 1U);
    ...
    // DTLS on, self-signed cert, no peer chain verification for demo
...
// ─────────────────────────────────────────────────────────────────────────────
// Helper: build and send a DTLS echo reply (extracts complexity from run_server)
// ─────────────────────────────────────────────────────────────────────────────
                             ...
                             const MessageEnvelope& received, uint64_t now_us)
{
    ...
    reply.reliability_class = ReliabilityClass::BEST_EFFORT;
    ...
    const Result res = engine.send(reply, now_us);
    if (!result_ok(res)) {
        LOG_WARN_LO("DtlsUdpDemo", "Echo send failed: %d\n", static_cast<int>(res));
    }
}
...
static int run_server(uint16_t port)
{
    ...
    LOG_INFO("DtlsUdpDemo", "Server starting on UDP port %u (DTLS)\n",
                static_cast<unsigned>(port));

    TransportConfig cfg;
    make_dtls_config(cfg, true, port);

    DtlsUdpBackend transport;
    const Result res = transport.init(cfg);
    if (!result_ok(res)) {
        LOG_FATAL("DtlsUdpDemo", "Server transport init failed: %d\n", static_cast<int>(res));
        return 1;
    }
    ...
    LOG_INFO("DtlsUdpDemo", "Server ready. Press Ctrl+C to stop.\n");
    ...
    transport.close();
    LOG_INFO("DtlsUdpDemo", "Server stopped. Messages received: %u\n", received);
...
static int run_client(uint16_t port)
{
    ...
    LOG_INFO("DtlsUdpDemo", "Client connecting to 127.0.0.1:%u (DTLS)\n",
                static_cast<unsigned>(port));

    // Wait for server to be ready and DTLS cookie exchange to complete
    usleep(static_cast<useconds_t>(CLIENT_STARTUP_US));

    TransportConfig cfg;
    make_dtls_config(cfg, false, port);

    DtlsUdpBackend transport;
    Result res = transport.init(cfg);
    if (!result_ok(res)) {
        LOG_FATAL("DtlsUdpDemo", "Client transport init failed: %d\n", static_cast<int>(res));
        return 1;
    }

    // REQ-6.1.10: DeliveryEngine::init() calls register_local_id() which sends
    // the HELLO datagram so the server can register our NodeId for routing.
    DeliveryEngine engine;
    ...
    for (int i = 0; i < MAX_CLIENT_MSGS; ++i) {
        ...
        char text[64];
        (void)snprintf(text, sizeof(text), "DTLS hello #%d from client", i + 1);
        ...
        if (result_ok(res)) {
            ...
            LOG_WARN_HI("DtlsUdpDemo", "Send failed: %d\n", static_cast<int>(res));
        }
...
int main(int argc, char* argv[])
{
    ...
    init_pem_paths();
    if (!write_pem_files()) {
        (void)fprintf(stderr, "ERROR: failed to write DTLS cert/key to /tmp\n");
        return 1;
    }
