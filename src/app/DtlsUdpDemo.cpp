...
static const char*    CERT_FILE = "/tmp/me_dtls_demo.crt";
static const char*    KEY_FILE  = "/tmp/me_dtls_demo.key";
                             ...
                             const MessageEnvelope& received, uint64_t now_us)
{
    ...
    if (!result_ok(res)) {
        Logger::log(Severity::WARNING_LO, "DtlsUdpDemo",
                    "Echo send failed: %d\n", static_cast<int>(res));
    }
}
...
static int run_server(uint16_t port)
{
    ...
    Logger::log(Severity::INFO, "DtlsUdpDemo",
                "Server starting on UDP port %u (DTLS)\n",
    ...
    if (!result_ok(res)) {
        Logger::log(Severity::FATAL, "DtlsUdpDemo",
...
static int run_client(uint16_t port)
{
    ...
    Logger::log(Severity::INFO, "DtlsUdpDemo",
                "Client connecting to 127.0.0.1:%u (DTLS)\n",
                static_cast<unsigned>(port));
    ...
    for (int i = 0; i < MAX_CLIENT_MSGS; ++i) {
        ...
        if (result_ok(res)) {
            ...
            Logger::log(Severity::WARNING_HI, "DtlsUdpDemo",
                        "Send failed: %d\n", static_cast<int>(res));
        }
