 ...
 *
 * Moved from src/platform/ to src/core/ so that ChannelConfig (a core type)
 * can embed a full ImpairmentConfig without creating a core → platform layering
 * violation. ImpairmentConfig has no platform dependencies (only <cstdint> and
 * core/Types.hpp) and therefore belongs in the core layer.
 *
...
#ifndef CORE_IMPAIRMENT_CONFIG_HPP
#define CORE_IMPAIRMENT_CONFIG_HPP
