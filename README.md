...
| Document | Normative policy | Source files | Other docs |
|---|---|---|---|
| `HAZARD_ANALYSIS.md` | `CLAUDE.md` §11, §13 | All `src/core/` and `src/platform/` `.hpp` files (SC/NSC classification tables) | — (primary artifact; others depend on it) |
| `STATE_MACHINES.md` | `CLAUDE.md` §15 | `AckTracker.hpp/cpp`, `RetryManager.hpp/cpp`, `ImpairmentEngine.hpp/cpp`, `Types.hpp` (capacity constants) | `HAZARD_ANALYSIS.md` §1 (HAZ IDs) |
| `TRACEABILITY_MATRIX.md` | `CLAUDE.md` §11, `.claude/CLAUDE.md` §10 | All `src/` and `tests/` files (`// Implements:` / `// Verifies:` comments); REQ IDs from `CLAUDE.md` §§3–7 | — (generated by `docs/check_traceability.sh`) |
| `STACK_ANALYSIS.md` | `CLAUDE.md` §13 | `src/app/` entry points; `DeliveryEngine`, `Serializer`, `AckTracker`, `RetryManager` (core); `TcpBackend`, `DtlsUdpBackend`, `ImpairmentEngine`, `IMbedtlsOps`, `MbedtlsOpsImpl` (platform) | — |
| `WCET_ANALYSIS.md` | `CLAUDE.md` §14 | `src/core/Types.hpp` (capacity constants); all SC function implementation files | `HAZARD_ANALYSIS.md` §3 (SC function scope); `STACK_ANALYSIS.md` (worst-case call chain) |
| `MCDC_ANALYSIS.md` | `CLAUDE.md` §14 §2 | `DeliveryEngine.cpp` (lines 77–222), `DuplicateFilter.cpp` (lines 44–110), `Serializer.cpp` (lines 117–259), `Assert.hpp`; `tests/test_DeliveryEngine.cpp`, `tests/test_DuplicateFilter.cpp`, `tests/test_Serializer.cpp` | `HAZARD_ANALYSIS.md` §3 (five highest-hazard SC functions); `VERIFICATION_POLICY.md` (architectural ceiling rules); `CLAUDE.md` §14 |
| `INSPECTION_CHECKLIST.md` | `CLAUDE.md` §12 | Files under review (varies per inspection) | `.claude/CLAUDE.md` §§2–5 (Power of 10, MISRA, F-Prime, architecture — Parts C–E); `CLAUDE.md` §§6–7, §11 (error handling, security, traceability — Parts B, F, G); `DEFECT_LOG.md` (completed checklists filed there) |
| `DEFECT_LOG.md` | `CLAUDE.md` §12.3 | Files reviewed in each inspection | `INSPECTION_CHECKLIST.md`; `HAZARD_ANALYSIS.md` (HAZ IDs); `VERIFICATION_POLICY.md` (verification method references) |
| `VERIFICATION_POLICY.md` | NPR 7150.2D, NASA-STD-8739.8A, NASA-STD-8719.13C | — (process/policy document) | `HAZARD_ANALYSIS.md` §3 (SC/NSC definitions); `CLAUDE.md` §14 (coverage requirements) |
