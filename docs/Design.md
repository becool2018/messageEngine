...
|---|---|---|---|
| `message_type` | `MessageType` | 1 byte | DATA / ACK / NAK / HEARTBEAT / HELLO / INVALID |
...
| *(proto_version)* | `uint8_t` | 1 byte | `PROTO_VERSION` byte; deserialized rejects mismatches |
...
| `payload_length` | `uint32_t` | 4 bytes | Bytes of valid data in `payload[]` for this fragment |
| `proto_magic` | `uint16_t` | 2 bytes | `PROTO_MAGIC` = 0x4D45 ('ME'); deserialized rejects mismatches |
| *(reserved)* | `uint16_t` | 2 bytes | Reserved; must be 0 on wire |
| `sequence_num` | `uint32_t` | 4 bytes | Per-sender sequence number; 0 = unordered / control |
| `fragment_index` | `uint8_t` | 1 byte | 0-based fragment index; 0 for unfragmented messages |
| `fragment_count` | `uint8_t` | 1 byte | Total fragments; 1 = unfragmented message |
| `total_payload_length` | `uint16_t` | 2 bytes | Logical payload size across all fragments |
| `payload` | `uint8_t[4096]` | 0–4096 bytes | Application data (or one fragment); opaque to all layers below App |

**Wire header total: 52 bytes** (`WIRE_HEADER_SIZE`). Payload follows immediately. Maximum wire frame: 52 + 4096 = 4,148 bytes.
...
- `ERR_IO` on the send path means a message was dropped (by impairment, socket error, or partition). For BEST_EFFORT this is expected and treated as a silent drop. For RELIABLE_RETRY, if the transport sends successfully but the network silently drops the packet (loss impairment, not ERR_IO), the retry manager will retransmit when the backoff timer fires. If `send_fragments()` returns `ERR_IO` because the **first** fragment failed (nothing reached the wire), bookkeeping slots are rolled back by `rollback_on_transport_failure()` — the caller receives `ERR_IO`. If `send_fragments()` returns `ERR_IO_PARTIAL` (at least one fragment was already transmitted before the failure), bookkeeping slots are **preserved**: the AckTracker deadline and RetryManager retry schedule remain active so the retry mechanism can retransmit the logical message when the backoff timer fires. `send()` normalises `ERR_IO_PARTIAL` to `ERR_IO` for the caller in both cases.
...
Static-methods-only class (not instantiable). Deterministic big-endian serialization with manual bit-shift helpers (`write_u8`, `write_u16`, `write_u32`, `write_u64`, `read_u8`, `read_u16`, `read_u32`, `read_u64`) and file-local validators (`message_type_in_range`, `reliability_class_in_range`, `fragment_header_valid`, `payload_length_valid`).
...
- Byte 3 carries `PROTO_VERSION`; byte 40–41 carry `PROTO_MAGIC`; bytes 42–43 are a reserved zero field. Deserialization rejects any mismatch.
- `payload_length` is bounds-checked against `MSG_MAX_PAYLOAD_BYTES` before the payload copy.
- v2 extension fields (`sequence_num`, `fragment_index`, `fragment_count`, `total_payload_length`) are always serialized; all-zero values indicate an unfragmented, unordered message.
...
#### 8.1.12 ProtocolVersion (`ProtocolVersion.hpp`)

Header-only constants: `PROTO_VERSION` (the version byte placed at wire offset 3) and `PROTO_MAGIC` (the 2-byte magic word placed at wire offsets 40–41). Both are compared on every deserialization call; a mismatch causes `ERR_INVALID` to be returned and the frame is discarded. Bumping `PROTO_VERSION` signals a wire format change; `PROTO_MAGIC` is fixed for the lifetime of the project.

---

#### 8.1.13 Fragmentation (`Fragmentation.hpp / .cpp`)

Stateless helper for oversized payloads. `fragment_message()` splits a logical `MessageEnvelope` with `payload_length > FRAG_MAX_PAYLOAD_BYTES` (1024) into up to `FRAG_MAX_COUNT` (4) fragment envelopes. Each fragment copies the header fields from the original, sets `fragment_index` (0-based) and `fragment_count`, and carries a slice of the payload. No internal state — safe to call concurrently from the same or different senders.

The Serializer and DeliveryEngine call `Fragmentation` transparently on the send path; callers above `DeliveryEngine::send()` never see fragments.

---

#### 8.1.14 ReassemblyBuffer (`ReassemblyBuffer.hpp / .cpp`)

Fixed table of `REASSEMBLY_SLOT_COUNT` (8) reassembly slots, each keyed on `(source_id, message_id)`. When the final fragment arrives, the complete logical envelope is reconstructed and made available via `get_complete()`. `sweep_stale(now_us)` reclaims any slot that has been open longer than `recv_timeout_ms` without completing (REQ-3.2.9), preventing resource exhaustion from partial fragment sets.

Design rationale: the slot count (8) is sized for `MAX_PEER_NODES / 2` simultaneous in-flight fragmented messages, which is the expected operational maximum.

---

#### 8.1.15 OrderingBuffer (`OrderingBuffer.hpp / .cpp`)

Per-peer sequencing gate for `ORDERED` delivery channels. Each peer has a `next_expected_seq` counter. An arriving envelope with `sequence_num == next_expected` is passed through immediately; out-of-sequence envelopes are buffered in a hold queue (up to `ORDERING_HOLD_COUNT` = 8 slots per peer, across `ORDERING_PEER_COUNT` = 16 peers). When the expected sequence arrives, the buffer drains in order.
...
Application-facing helper for synchronous request/reply semantics. Wraps `DeliveryEngine` and serializes a `RequestReplyHeader` at the front of each outbound payload. A pending-requests table (fixed size) holds per-request state: correlation ID, timeout deadline, and callback pointer. `pump()` matches incoming DATA envelopes by `request_id` and invokes the registered callback.

Design rationale: request/reply is implemented as a thin layer above `DeliveryEngine` rather than embedded in it, preserving `DeliveryEngine`'s single-responsibility scope and keeping it testable in isolation.

---

#### 8.1.17 DeliveryEvent / DeliveryEventRing (`DeliveryEvent.hpp`, `DeliveryEventRing.hpp`)

Pull-style observability for `DeliveryEngine`. After each significant delivery decision, `DeliveryEngine` pushes a `DeliveryEvent` (kind, source_id, message_id, timestamp_us) to the owned `DeliveryEventRing`. The ring capacity is `DELIVERY_EVENT_RING_CAPACITY` (64); on overflow the oldest event is overwritten. Consumers call `poll_event()` or `drain_events()` — no callbacks, no blocking.

This satisfies REQ-7.2.5 without dynamic allocation, threads, or callbacks in the core layer.
...
#### 8.2.8 DtlsUdpBackend (`DtlsUdpBackend.hpp / .cpp`)

`TransportInterface` over DTLS-encrypted UDP, implementing REQ-6.4.1 through REQ-6.4.6:

- **DTLS cookie anti-replay (REQ-6.4.2):** Server calls `mbedtls_ssl_cookie_setup()` and generates a cookie bound to `(client_ip, client_port)` before completing the handshake. `mbedtls_ct_memcmp()` is used for timing-safe cookie comparison (§7d of `.claude/CLAUDE.md`).
- **Retransmission timer (REQ-6.4.3):** `mbedtls_timing_delay_context` drives DTLS retransmission without busy-waiting; min/max intervals match expected link RTT.
- **MTU enforcement (REQ-6.4.4):** Serialized frames exceeding `DTLS_MAX_DATAGRAM_BYTES` (1400) are rejected before encryption.
- **Plaintext fallback (REQ-6.4.5):** When `tls_enabled = false`, operates as a plain UDP backend without any mbedTLS state.
- **Peer hostname verification (REQ-6.4.6):** Client mode calls `mbedtls_ssl_set_hostname()` when `peer_hostname` is non-empty.

`process_hello_or_validate()` enforces HELLO-before-data and `source_id` consistency (SC: HAZ-009, HAZ-011). Injectable `IMbedtlsOps` seam enables M5 fault-injection testing of all mbedTLS error paths.

---

#### 8.2.9 ImpairmentConfigLoader (`ImpairmentConfigLoader.hpp / .cpp`)

Free function `impairment_config_load(path, cfg)`. Reads up to `MAX_CONFIG_LINES` (64) key=value lines from an INI-style text file. Each line is dispatched to one of three topic handlers (`apply_kv_latency_jitter`, `apply_kv_loss_reorder`, `apply_kv_partition_seed`) and five type-safe parse helpers (`parse_uint`, `parse_bool`, `parse_prob`, `parse_u64`, `apply_reorder_window`). Unknown keys are logged at WARNING_LO and ignored; probability fields are clamped to [0.0, 1.0]. Returns ERR_IO only on `fopen()` failure — all parse errors are handled gracefully. SC: HAZ-002, HAZ-007.
...
### 9.5 ReassemblyBuffer Slot State Machine

| State | Condition | Transition |
|---|---|---|
| `SLOT_FREE` | Slot not in use | → `SLOT_COLLECTING` on first `add_fragment()` for a new `(source_id, message_id)` |
| `SLOT_COLLECTING` | Waiting for all `fragment_count` fragments | → `SLOT_COMPLETE` when all fragments received |
| `SLOT_COLLECTING` | Open longer than `recv_timeout_ms` | → `SLOT_FREE` on `sweep_stale()` (reclaimed) |
| `SLOT_COMPLETE` | All fragments present | → `SLOT_FREE` on `get_complete()` |

**Invariant:** Slot is `SLOT_COMPLETE` if and only if `fragments_received == fragment_count`.

---

### 9.6 OrderingBuffer Peer State

| Sub-state | Condition | Transition |
|---|---|---|
| Holding | `env.sequence_num != next_expected` AND hold queue not full | → hold queue; advance when expected arrives |
| Gap declared | Hold queue full before expected arrives | Oldest held dropped; gap logged; `next_expected` advanced |
| Pass-through | `env.sequence_num == next_expected` | Increment `next_expected`; drain any held envelopes in sequence |

**Invariant:** `next_expected` for a peer is monotonically non-decreasing.

---

## 10. Wire Format

Protocol v2 — 52-byte fixed header followed by variable payload.
...
2        1      priority              (uint8)
3        1      PROTO_VERSION         (uint8; constant — rejected if unrecognised)
...
36       4      payload_length        (uint32, big-endian; bytes in this fragment)
40       2      PROTO_MAGIC           (uint16, big-endian; 0x4D45 = 'ME')
42       2      reserved              (must be 0x0000)
44       4      sequence_num          (uint32, big-endian; 0 = unordered/control)
48       1      fragment_index        (uint8; 0 = unfragmented or first fragment)
49       1      fragment_count        (uint8; 1 = unfragmented message)
50       2      total_payload_length  (uint16, big-endian; logical size across all fragments)
52       N      payload               (N = payload_length, 0 ≤ N ≤ 4096)
```

Total header size: **52 bytes** (`Serializer::WIRE_HEADER_SIZE`). Maximum wire frame: 52 + 4096 = 4,148 bytes.
...
- `buf_len >= WIRE_HEADER_SIZE (52)` — rejects truncated headers.
- `version byte == PROTO_VERSION` — rejects protocol version mismatches.
- `magic word == PROTO_MAGIC (0x4D45)` — rejects malformed or misidentified frames.
- `reserved == 0x0000` — rejects frames with unexpected reserved bits set.
...
| `test_RingBuffer` | `RingBuffer` SPSC queue | 8 |
...
|---|---|---|
| `MSG_MAX_PAYLOAD_BYTES` | 4,096 | Maximum application payload per message (or per fragment) |
...
| `SOCKET_RECV_BUF_BYTES` | 8,192 | Wire receive buffer (≥ 52 + 4096 = 4,148 required) |
| `NODE_ID_INVALID` | 0 | Sentinel value; never a valid `NodeId` |
| `DTLS_MAX_DATAGRAM_BYTES` | 1,400 | DTLS record MTU; serialized frame must not exceed this before encryption |
| `DELIVERY_EVENT_RING_CAPACITY` | 64 | `DeliveryEventRing` capacity (overwrite-on-full) |
| `FRAG_MAX_PAYLOAD_BYTES` | 1,024 | Maximum payload per fragment envelope |
| `FRAG_MAX_COUNT` | 4 | Maximum fragments per logical message |
| `REASSEMBLY_SLOT_COUNT` | 8 | Simultaneous in-flight reassembly entries |
| `ORDERING_PEER_COUNT` | 16 | Per-peer ordering state entries (`= MAX_PEER_NODES`) |
| `ORDERING_HOLD_COUNT` | 8 | Out-of-order envelopes held per peer before gap declaration |
...
### 15.7 Fragmentation at the Serializer / DeliveryEngine boundary

**Decision:** Fragmentation is applied transparently by `DeliveryEngine::send()` before calling `transport->send_message()`. Callers above the DeliveryEngine never see fragments.

**Rationale:** This keeps the `TransportInterface` contract simple (one send = one wire frame) while allowing large payloads to traverse MTU-constrained links (e.g., DTLS at 1400 bytes). Fragmentation state (`fragment_index`, `fragment_count`, `sequence_num`) is carried in every wire header, not just in fragmented messages, so the deserializer path is uniform.

---

### 15.8 TLS/DTLS as a transparent backend swap

**Decision:** `TlsTcpBackend` and `DtlsUdpBackend` implement the same `TransportInterface` as `TcpBackend` and `UdpBackend` respectively. When `tls_enabled = false` they operate as plain TCP/UDP backends.

**Rationale:** REQ-6.3.4 requires secure and insecure transports to be swappable without changing higher layers. Making TLS/DTLS a `TransportInterface` implementation keeps `DeliveryEngine` unchanged and avoids conditional TLS paths in core code.

---

### 15.9 Injectable interfaces for mbedTLS and POSIX socket operations

**Decision:** `IMbedtlsOps` (for `DtlsUdpBackend`) and `ISocketOps` (for `TcpBackend` / `UdpBackend`) are pure-virtual injectable interfaces. Production code uses singleton implementations; tests inject mock implementations.

**Rationale:** VVP-001 §4.4 (injectable interface requirement) mandates that every module verified to Class B or Class A standard must provide an injectable interface for every external dependency whose failure constitutes an error-handling path. mbedTLS calls (`ssl_handshake`, `ssl_read`, `ssl_write`) and POSIX socket calls (`accept`, `recv`, `send`) cannot fail in a loopback test environment, so M5 fault injection requires injectable seams. The injected singleton pattern preserves the no-dynamic-allocation rule: the backend holds an `ISocketOps*` or `IMbedtlsOps*` that is set at construction time (not heap-allocated).
