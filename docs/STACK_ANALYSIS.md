...
**DTLS dual-buffer hazard:** `DtlsUdpBackend::send_message()` itself also declares a
`MessageEnvelope delayed[IMPAIR_DELAY_BUF_SIZE]` local array (a second, independent copy)
in addition to the one declared inside `flush_delayed_to_wire()`. When both frames are live
simultaneously on the call stack (i.e., `send_message()` calls into `flush_delayed_to_wire()`),
the DTLS outbound path carries **two** such arrays concurrently — a combined stack allocation
of **2 × 132,608 = 265,216 bytes (~259 KB)**. This is the true worst-case peak for Chain 5
(DTLS outbound send with flush active), not ~130 KB. See Chain 5 below.
...
above. The DTLS outbound path (Chain 5) carries an additional hazard: both
`DtlsUdpBackend::send_message()` and `flush_delayed_to_wire()` declare independent
`delayed[]` arrays, doubling the flush-path stack cost to ~259 KB when both frames are live.
...
**Depth:** 12 frames  ← **worst-case frame depth across all chains**
...
main()  [~64 B]
  └─ run_client_iteration()  [~80 B]
       └─ send_test_message()  [~300 B]  ← large: payload_buf[256] on stack
            └─ DeliveryEngine::send()  [~64 B]
                 └─ DeliveryEngine::send_via_transport()  [~48 B]
                      └─ DtlsUdpBackend::send_message()  [~132,608 B]  ← MessageEnvelope delayed[32] declared here
                           └─ Serializer::serialize()  [~32 B]
                                └─ ImpairmentEngine::process_outbound()  [~80 B]
                                     └─ ImpairmentEngine::queue_to_delay_buf()  [~32 B]
```

When `flush_delayed_to_wire()` is invoked from within `send_message()`, a second
`MessageEnvelope delayed[IMPAIR_DELAY_BUF_SIZE]` array is declared in `flush_delayed_to_wire()`'s
own frame while `send_message()`'s copy remains live on the stack. The flush path continues via
`IMbedtlsOps::ssl_write()` (one additional frame for the `MbedtlsOpsImpl::ssl_write()`
thin-wrapper, ~16 B). **Both** `delayed[]` arrays are simultaneously live during that call,
making the true worst-case peak for the DTLS flush path approximately **2 × 132,608 = 265,216
bytes (~259 KB)**, not ~130 KB.
...
**Depth:** 10 frames (with `MbedtlsOpsImpl::ssl_write()` virtual dispatch on flush path; 9 without)
**Estimated peak stack (our code, non-flush path):** ~764 B
**Estimated peak stack (our code, flush path — dual delayed[] arrays live):** ~265,216 B (~259 KB)
...
is 5 frames deep, well below the 12-frame worst case of Chain 3. It does **not** represent
            ...
            └─ DeliveryEngine::receive()  [~80 B]
                 └─ DtlsUdpBackend::receive_message()  [~80 B]
                      └─ DtlsUdpBackend::recv_one_dtls_datagram()  [~48 B]
                           └─ DtlsUdpBackend::try_tls_recv()  [~32 B]
                                └─ MbedtlsOpsImpl::ssl_read()  [~16 B]  ← IMbedtlsOps virtual dispatch
                                     └─ DtlsUdpBackend::deserialize_and_dispatch()  [~48 B]
                                          └─ Serializer::deserialize()  [~32 B]
```

**Depth:** 10 frames (two additional helper frames: `try_tls_recv()` and `deserialize_and_dispatch()`)
...
| 3 — Retry pump (with flush) | **12** | **~132,608 B (~130 KB)** |
| 4 — ACK timeout sweep | 6 | ~352 B |
| 5 — DTLS outbound send (with flush, dual delayed[]) | **10** | **~265,216 B (~259 KB)** |
| 6 — DTLS inbound receive | 10 | ~568 B |
| 7 — HELLO registration (init-phase only) | 5–6 | ~256 B |
| **Worst case (frame depth)** | **12 (Chain 3 — retry pump with send_fragments)** | **~132,608 B (~130 KB)** |
| **Worst case (stack size)** | **10 (Chain 5 — DTLS flush with dual delayed[] arrays)** | **~265,216 B (~259 KB)** |

The worst-case **frame depth** is **12 frames** (Chain 3 — retry pump with `send_fragments()`
inserted between `send_via_transport()` and `TcpBackend::send_message()`). The worst-case
**stack size** is **~259 KB** (Chain 5 — DTLS outbound flush path where `send_message()` and
`flush_delayed_to_wire()` each declare a `MessageEnvelope delayed[IMPAIR_DELAY_BUF_SIZE]` array
and both are simultaneously live). For non-DTLS, non-flush paths the worst-case stack is the
"~764 B" non-flush estimate.
...
| Platform | Default per-thread stack | Headroom vs. worst case (~259 KB, Chain 5 DTLS dual delayed[]) |
|----------|--------------------------|----------------------------------------------------------------|
| macOS | 8 MB (main), 512 KB (pthreads) | ~31× (main) / ~2× (pthreads) |
| Linux | 8 MB (main), 8 MB (pthreads) | ~31× |

No stack overflow risk exists on macOS/Linux at the default stack sizes for the main thread.
However, the **~259 KB** DTLS flush worst-case (Chain 5, dual `delayed[]` arrays) makes this
codebase **unsuitable for embedded targets with ≤ 512 KB stack** without first restructuring
`DtlsUdpBackend::send_message()` and `flush_delayed_to_wire()` so that at most one
`MessageEnvelope delayed[32]` is live on the stack at any time.

pthreads on macOS default to 512 KB; the DTLS flush path leaves only ~2× headroom on that
configuration — insufficient margin for safe deployment. Consider calling
`pthread_attr_setstacksize()` with ≥ 2 MB for application threads that invoke the DTLS
message send path. For non-DTLS (TCP/TLS) backends the single `delayed[]` flush path of
~130 KB still leaves ~4× headroom on a 512 KB pthread stack.
...
- A new call chain adds frames beyond depth 12.
