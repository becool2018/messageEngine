...
| S-D7 | `send()` | L603 | `res != Result::OK` | A = `res != OK` | A=T: delegate to `handle_send_fragments_failure()` — see S-D8/S-D9; return error | A=F: increment msgs_sent; return OK | T: `test_mock_send_transport_err_io`, `test_mock_fragmented_partial_send_keeps_bookkeeping`; F: `test_send_best_effort` |
| S-D8 | `handle_send_fragments_failure()` | L537 | `res == Result::ERR_IO_PARTIAL` | A = `res == ERR_IO_PARTIAL` | A=T: preserve bookkeeping slots (≥1 fragment already on wire); return ERR_IO | A=F: proceed to reliability-class rollback check (S-D9) | T: `test_mock_fragmented_partial_send_keeps_bookkeeping`; F: `test_mock_send_transport_err_io` |
| S-D9 | `handle_send_fragments_failure()` | L544 | `rel == RELIABLE_ACK \|\| rel == RELIABLE_RETRY` | A = `rel == RELIABLE_ACK`; B = `rel == RELIABLE_RETRY` | A=T or B=T → call rollback_on_transport_failure() (cancel slots) | A=F, B=F (BEST_EFFORT) → skip rollback | T(A): `test_mock_reliable_ack_send_err_rolls_back_ack_tracker`; T(B): `test_mock_reliable_retry_send_err_rolls_back_both_slots`; F: `test_mock_send_transport_err_io` |
