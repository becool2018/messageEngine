...
| HAZ-010 | Predictable message_id sequence enables ACK forgery — MessageIdGen was seeded from local_id only (a small, known uint32_t). An attacker who knows the node ID can predict all future message_id values and send forged ACKs, prematurely clearing AckTracker/RetryManager slots and causing the sender to believe messages were delivered when they were not (silent message loss on RELIABLE_RETRY channels). | Cat I | MessageIdGen seeded from low-entropy local_id in DeliveryEngine::init() | Seed XORs local_id<<32 with timestamp_now_us() in DeliveryEngine::init() (DEF-014-2); reduces predictability to knowledge of exact process start time |
