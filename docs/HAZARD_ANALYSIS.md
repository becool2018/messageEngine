...
| HAZ-016 | **Ordering gate permanent stall on peer reconnect** — two independent mechanisms can permanently stall an ordered delivery channel after a peer reconnects: (a) inbound: `OrderingBuffer` retains the old `next_expected_seq` (e.g., 847); the new connection starts sending from seq=1, which is discarded as a duplicate, stalling the channel for the lifetime of the new session; (b) outbound: `DeliveryEngine::m_seq_state` retains the old `next_seq` (e.g., 6) for the destination; the reconnecting peer starts its receive-side `OrderingBuffer` at `expected=1` but receives seq=6 on the first message, permanently stalling the peer's ordered delivery channel. | Cat II | (a) `OrderingBuffer::ingest()` discards `seq < next_expected_seq` as a duplicate; on reconnect the new session's messages start from seq=1, which is always below the retained value from the prior session. (b) `DeliveryEngine::next_seq_for(dst)` advances `m_seq_state[].next_seq` monotonically; the slot is never reset to 1 on reconnect, so the peer's receiver — which starts at `expected=1` — holds all messages as out-of-order indefinitely. (c) `AckTracker` and `RetryManager` retain PENDING/WAITING slots for the reconnecting peer; these retry pre-reset envelopes (with stale sequence numbers from the prior session) on the new connection, inserting old sequence numbers into the new session's ordered delivery stream. | (a) `OrderingBuffer::reset_peer(src)` resets `next_expected_seq` to 1 and frees all held slots for `src`. (b) `reset_peer_ordering(src)` also iterates `m_seq_state[]` and resets `next_seq` to 1 for the slot where `dst == src`, so the sender restarts its outbound sequence to match the reconnecting peer's receive-side expectation. `DeliveryEngine::reset_peer_ordering(src)` additionally discards any staged `m_held_pending` message belonging to `src`. (c) `reset_peer_ordering(src)` calls `AckTracker::cancel_peer(src)` and `RetryManager::cancel_peer(src)` to cancel all PENDING/WAITING slots for the reconnecting peer's destination, emitting one `RECONNECT_CANCEL` observability event per cancelled entry. `DeliveryEngine::drain_hello_reconnects()` polls `TransportInterface::pop_hello_peer()` at the top of every `receive()` call and triggers `reset_peer_ordering()` for each reconnecting peer (REQ-3.3.6). |
...
| `cancel_peer(dst)` not called on peer reconnect | WAITING/active slots for old connection remain; `collect_due()` later fires stale retries with pre-reset sequence numbers on new connection, inserting old sequence numbers into new-session ordered delivery stream | Cat II HAZ-016 | `reset_peer_ordering()` calls `cancel_peer(src)` on every HELLO reconnect event | `RetryManager::cancel_peer(src)` added (INSP-034); iterates all slots, deactivates any active entry where `dst == src`; returns count cancelled; called from `reset_peer_ordering()` |
...
| `cancel_peer(dst)` not called on peer reconnect | PENDING slots for old connection remain active; `sweep_expired()` later retries stale envelopes with pre-reset sequence numbers on new connection, corrupting new-session ordered delivery | Cat II HAZ-001, HAZ-016 | `reset_peer_ordering()` calls `cancel_peer(src)` on every HELLO reconnect event | `AckTracker::cancel_peer(src)` added (INSP-034); iterates all slots, cancels any PENDING entry where `dst == src`; returns count cancelled; called from `reset_peer_ordering()` |
...
| `m_seq_state` outbound seq not reset on peer reconnect | Reconnecting peer receives seq=N+1 on its first message while its `OrderingBuffer` expects seq=1; all messages held as out-of-order; peer's ordered channel permanently stalled for the lifetime of the new session | Cat II HAZ-016 | Observed: second client run receives 0/5 echo replies, all held in ordering buffer | `reset_peer_ordering(src)` now iterates `m_seq_state[]` and resets `next_seq` to 1 for the slot where `dst == src`, ensuring the sender's outbound sequence restarts to match the reconnecting peer's receive-side expectation |
...
| `cancel_peer()` | `AckTracker` | SC | HAZ-001, HAZ-016 |
...
| `cancel_peer()` | `RetryManager` | SC | HAZ-016 |
...
`cancel_peer()` is SC (HAZ-016): peer-reconnect stale-slot cancellation; it deactivates all WAITING slots for a reconnecting peer's destination (`dst == src`). Failure to call this on reconnect leaves stale retry entries that fire pre-reset sequence-numbered envelopes on the new connection, permanently stalling the new session's ordered delivery channel.
